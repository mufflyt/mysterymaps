...
test_that("Inf is excluded from the breaks AND shaded as no data", {
  ...
  # independent -- the filter protects the breaks, the colour protects the
  # reader.
  ...
  # And Inf itself is no data, not the top class.
  expect_identical(sc$color(Inf), sc$color(NA_real_))
  expect_false(sc$color(Inf) %in% sc$leg_cols[-c(1, length(sc$leg_cols))])
})
